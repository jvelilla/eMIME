	...
	fitness_and_quality_parsed ( a_mime_type : STRING; parsed_ranges : LIST[PARSE_RESULTS]) : FITNESS_AND_QUALITY
		...
		do
			...
			loop
				...
				if (target.type.is_equal (range.type) or range.type.is_equal ("*") or target.type.is_equal ("*")) and
				   (target.sub_type.is_equal (range.sub_type) or range.sub_type.is_equal ("*") or target.sub_type.is_equal("*"))
				then
					from
						keys := target.keys
						keys.start
					...
					loop
						...
						if range.type.is_equal (target.type) then
							l_fitness := 100
						else
							l_fitness := 0
						end

						if range.sub_type.is_equal(target.sub_type) then
